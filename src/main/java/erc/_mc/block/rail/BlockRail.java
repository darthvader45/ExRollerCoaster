...
public class BlockRail extends BlockContainer{
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase placer, ItemStack itemStack)
	{
		if(world.isRemote)
		{
			...
			connectToSavedRailsFromClient(x, y, z);
		}
		else
		{
			// �T�[�o�[�̓��[��TileEntity�ݒu
			...
			TileEntityRail tileEntityRail = GetTileEntityInstance();
			onTileEntityInitFirst(world, placer, tileEntityRail, x, y, z);
			world.setTileEntity(x, y, z, tileEntityRail);
...
//	}
	...
	protected void connectToSavedRailsFromClient(int x, int y, int z)
	{
		if(NotifyConnecttionRailManager.isSavedPrevRail())
		{
			ERC_MessageConnectRailCtS packet
					= new ERC_MessageConnectRailCtS(
					NotifyConnecttionRailManager.savedPrevX,
					NotifyConnecttionRailManager.savedPrevY,
					NotifyConnecttionRailManager.savedPrevZ,
					x, y, z
			);
			ERC_PacketHandler.INSTANCE.sendToServer(packet);
		}

		NotifyConnecttionRailManager.SetPrevRailPosConnectedDestroyBlock(x, y, z);

		if(NotifyConnecttionRailManager.isSavedNextRail())
		{
			ERC_MessageConnectRailCtS packet
					= new ERC_MessageConnectRailCtS(
					x, y, z,
					NotifyConnecttionRailManager.savedNextX,
					NotifyConnecttionRailManager.savedNextY,
					NotifyConnecttionRailManager.savedNextZ
			);
			ERC_PacketHandler.INSTANCE.sendToServer(packet);
