...
public class ERC_MessageConnectRailCtS implements IMessage, IMessageHandler<ERC_MessageConnectRailCtS, IMessage> {
    ...
    public IMessage onMessage(ERC_MessageConnectRailCtS message, MessageContext ctx)
    {
        ...
        {
        	NextRail.getRail().SetPrevRail(BaseRail.getRail());
        	BaseRail.getRail().SetNextRail(NextRail.getRail());
			NextRail.getRail().ConstructCurve();
			BaseRail.getRail().ConstructCurve();
			BaseRail.SyncData();
            NextRail.SyncData();
		}
        return null;
